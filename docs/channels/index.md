...
- [Buzz](/channels/buzz) - Connect OpenClaw agents to Buzz rooms (bundled plugin).
- [ClickClack](/channels/clickclack) - ClickClack bot-token channel setup and target syntax (bundled plugin).
- [Discord](/channels/discord) - Discord bot setup, config keys, components, voice, and troubleshooting (bundled plugin).
- [Feishu](/channels/feishu) - Feishu bot overview, features, and configuration (bundled plugin).
- [Google Chat](/channels/googlechat) - Google Chat app support status, capabilities, and configuration (bundled plugin).
- [iMessage](/channels/imessage) - Native iMessage support via imsg (JSON-RPC over stdio), with private API actions for replies, tapbacks, effects, polls, attachments, and group management. Preferred for new OpenClaw iMessage setups when host requirements fit (bundled plugin).
- [IRC](/channels/irc) - IRC plugin setup, access controls, and troubleshooting (bundled plugin).
- [LINE](/channels/line) - LINE Messaging API plugin setup, config, and usage (bundled plugin).
- [Matrix](/channels/matrix) - Matrix support status, setup, and configuration examples (bundled plugin).
- [Mattermost](/channels/mattermost) - Mattermost bot setup and OpenClaw config (bundled plugin).
- [Microsoft Teams](/channels/msteams) - Microsoft Teams bot support status, capabilities, and configuration (bundled plugin).
- [Nextcloud Talk](/channels/nextcloud-talk) - Nextcloud Talk support status, capabilities, and configuration (bundled plugin).
- [Nostr](/channels/nostr) - Nostr DM channel via NIP-04 encrypted messages (bundled plugin).
- [QQ bot](/channels/qqbot) - QQ Bot setup, config, and usage (official plugin).
- [Raft](/channels/raft) - Raft External Agent support through the Raft CLI wake bridge (bundled plugin).
...
- [Signal](/channels/signal) - Signal support via signal-cli (native daemon or bbernhard container), setup paths, and number model (bundled plugin).
- [Slack](/channels/slack) - Slack setup and runtime behavior (Socket Mode, HTTP Request URLs, and relay mode) (bundled plugin).
- [SMS](/channels/sms) - Twilio SMS/MMS setup, access controls, webhooks, and delivery status (bundled plugin).
- [Synology Chat](/channels/synology-chat) - Synology Chat webhook setup and OpenClaw config (bundled plugin).
- [Telegram](/channels/telegram) - Telegram bot support status, capabilities, and configuration (bundled plugin).
- [Tlon](/channels/tlon) - Tlon/Urbit support status, capabilities, and configuration (bundled plugin).
- [Twitch](/channels/twitch) - Twitch chat bot: install, credentials, access control, token refresh (bundled plugin).
...
- [WhatsApp](/channels/whatsapp) - WhatsApp channel support, access controls, delivery behavior, and operations (bundled plugin).
- [Yuanbao](/channels/yuanbao) - Yuanbao bot overview, features, and configuration (external plugin).
- [Zalo](/channels/zalo) - Zalo bot support status, capabilities, and configuration (bundled plugin).
...
- [Zalo personal](/channels/zalouser) - Zalo personal account support via native zca-js (QR login), capabilities, and configuration (bundled plugin).
