...
describe("buildBareSessionResetPrompt", () => {
  ...
  it("prefers bootstrap-pending wording over custom sessionResetPrompt when bootstrap is pending", () => {
    const cfg = {
      agents: { defaults: { sessionResetPrompt: "Custom greeting!" } },
    } as OpenClawConfig;
    const nowMs = Date.UTC(2026, 2, 3, 14, 0, 0);
    const prompt = buildBareSessionResetPrompt(cfg, nowMs, "full");
    expect(prompt).not.toContain("Custom greeting!");
    expect(prompt).toContain("while bootstrap is still pending for this workspace");
  });
