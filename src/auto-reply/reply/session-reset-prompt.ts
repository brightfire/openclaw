...
export function buildBareSessionResetPrompt(
  ...
  // When bootstrap is pending, use bootstrap-specific prompts — custom override does not apply.
  const promptBase =
    bootstrapMode === "full"
  ...
  return appendCronStyleCurrentTimeLine(promptBase, cfg ?? {}, nowMs ?? Date.now());
}
