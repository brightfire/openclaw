...
describe("resolveSession rollover archiving", () => {
  it("archives the existing session entry when a rollover creates a new session", async () => {
    ...
    await withTempStore({ [sessionKey]: existingEntry }, (storePath) => {
      const cfg = {
        session: {
          store: storePath,
          mainKey: "main",
      ...
      // Verify the archive entry was created in the store
      const archiveKey = `${sessionKey}:archived:${oldSessionId}`;
      const archived = result.sessionStore?.[archiveKey];
      expect(archived).toBeDefined();
      expect(archived?.archived).toBe(true);
      expect(archived?.archivedReason).toBe("rollover");
