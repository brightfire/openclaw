...
export function createSessionsHistoryTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      // When includeArchived is set and normal resolution fails, bypass the
      // error and pass the raw sessionKey directly to chat.history — its
      // archive fallback logic will scan for .jsonl.reset.* files.
      ...
      if (!resolvedSession.ok) {
        ...
        // Archive passthrough: use raw input as the session key
        resolvedKey = sessionKeyParam;
        displayKey = sessionKeyParam;
      } else {
